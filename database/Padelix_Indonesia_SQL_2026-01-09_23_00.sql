...
-- Last modification date: 2026-01-09 15:59:52.402
...
CREATE TABLE `portfolio_images` (
    `id` int  NOT NULL COMMENT 'unique primary identifier.',
    `portfolio_id` int  NOT NULL COMMENT 'Reference to the portofolio that have the image.',
    `is_primary` boolean  NOT NULL COMMENT 'If true, this is the main image used in thumbnails.',
    `image_url` varchar(255)  NOT NULL COMMENT 'Absolute path to the image (Storage or Public).',
    `alt_text` varchar(150)  NULL COMMENT 'Image''''s alt text',
    `sort_order` int  NOT NULL COMMENT 'Controls the sequence in the image gallery.',
    `created_at` datetime  NOT NULL COMMENT 'Standard audit timestamps.',
    `updated_at` datetime  NOT NULL COMMENT 'Standard audit timestamps.',
    CONSTRAINT `portfolio_images_pk` PRIMARY KEY (`id`)
) COMMENT 'Gallery of photos showcasing the specific portfolio project.';
...
CREATE TABLE `product_variants` (
    ...
    `price_adjustment` decimal(10,2)  NOT NULL DEFAULT 0.00 COMMENT 'Amount added to or subtracted from the product''''s base price.',
    `stock_quantity` decimal(10,2)  NOT NULL DEFAULT 0 COMMENT 'Current inventory level.',
...
CREATE TABLE `products` (
    ...
    `base_price` decimal(10,2)  NOT NULL DEFAULT 0.00 COMMENT 'Starting price before variant adjustments.',
    `created_at` datetime  NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Standard audit timestamps.',
    `updated_at` datetime  NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Standard audit timestamps.',
...
CREATE TABLE `users_roles` (
    ...
    `roles_id` int  NOT NULL COMMENT 'unique primary identifier.',
    `assigned_at` datetime  NOT NULL COMMENT 'Standard audit timestamps.',
...
-- Reference: portfolio_images_portfolios (table: portfolio_images)
ALTER TABLE `portfolio_images` ADD CONSTRAINT `portfolio_images_portfolios` FOREIGN KEY `portfolio_images_portfolios` (`portfolio_id`)
