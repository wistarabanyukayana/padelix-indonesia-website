  ...
  try {
    ...
    const ensureObject = (val: unknown): MediaMetadata => {
      ...
      if (typeof val === "object" && val !== null && "0" in val && "1" in val) {
        console.warn(
          "[Mux Webhook] Detected corrupted indexed-string metadata, attempting recovery",
        );
        try {
    ...
    const uploadId =
      data.upload_id || (type.startsWith("video.upload") ? data.id : null);
    ...
    if (assetId) {
      const res = await db
        .select()
        .from(medias)
        .where(eq(medias.fileKey, assetId))
        .limit(1);
      mediaRecord = res[0];
    }
    ...
    console.log(
      `[Mux Webhook] Processing event: ${type} for media ID: ${mediaRecord.id}`,
    );
    await createAuditLog(
      `WEBHOOK_MUX_${type.replace(/\./g, "_").toUpperCase()}`,
      mediaRecord.id,
      `Processing Mux event: ${type}`,
    );
    ...
    if (
      ...
      console.log(
        `[Mux Webhook] Linked media ${mediaRecord.id} to asset ${assetId}`,
      );
    }
    ...
    if (type === "video.asset.ready" || type === "video.asset.errored") {
      ...
      console.log(
        `[Mux Webhook] Media ${mediaRecord.id} is now ${data.status}`,
      );
    }
    ...
    if (type === "video.asset.deleted") {
      ...
      console.log(
        `[Mux Webhook] Deleted media ${mediaRecord.id} due to Mux deletion`,
      );
    }
